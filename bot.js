...
// ---------------------- IDŐ ÁTALAKÍTÓ ----------------------
function ms(time) {
  const timeMatch = time.match(/^(\d+)([dhms])$/i);
...
client.on("messageCreate", async (message) => {
  ...
  // ---------------------- ANTI-LINK SZŰRŐ ----------------------
  const linkPattern = /(https?:\/\/)?(www\.)?(discord\.gg|discord\.com\/invite)\/[a-zA-Z0-9]+/i;
  ...
  if (commandName === "nyroledel") {
    ...
    giveawayRoles[message.guild.id] = giveawayRoles[message.guild.id].filter(r => r !== role.id);
    saveRoles();
    return message.reply(`🗑️ Eltávolítva a nyereményjáték admin rangok közül: ${role}`);
  }
  ...
  if (commandName === "nyeremenyjatek") {
    ...
    if (subcommand === "del") {
      if (!giveawayChannels[message.guild.id]) {
        return message.reply("❌ Nincs beállítva nyereményjáték csatorna ezen a szerveren.");
      }
    ...
    if (subcommand === "start") {
      ...
      if (!channelId)
        return message.reply("❌ Először állítsd be a nyereményjáték csatornát: `.nyeremenyjatek set #csatorna`");
      ...
      if (!winnerCountMatch) return message.reply("❌ Használd a 'fő' végződést (pl: 1fő)");

      const timeMatch = timeArg.match(/^(\d+)([dhms])$/i);
      if (!timeMatch) return message.reply("❌ Érvénytelen időformátum! (pl: 1h, 30m, 10s)");
      ...
      const updateInterval = setInterval(async () => {
        ...
        const updatedEmbed = new EmbedBuilder()
          .setTitle("<a:nyeremenyjatek:1419291413127888956> Nyereményjáték")
          .setDescription(
            `**Nyeremény:** ${prize}\n**Nyertesek száma:** ${winnerCount}\n**Hátralévő idő:** ${formatTime(remaining)}\n\nKattints a "Csatlakozom" gombra!`
          )
          .setColor("#d18be2")
          .setTimestamp(endTime);
        ...
        try {
          ...
          console.log("Embed frissítési hiba:", error);
        }
      }, 1000);
      ...
      const endGiveaway = async () => {
        ...
        try {
          ...
          if (winners.length > 0) {
            await giveawayChannel.send(`<a:medal:1419795403552985248> **Gratulálok a nyerteseknek!**\n${winnerText}`);
          } else {
            await giveawayChannel.send("😢 Senki sem vett részt a nyereményjátékban.");
          }
        } catch (error) {
      ...
      collector.on('collect', async i => {
        if (participants.has(i.user.id)) {
          await i.reply({ content: '❌ Már részt veszel!', ephemeral: true });
          return;
        }
        participants.add(i.user.id);
        await i.reply({ content: '✅ Sikeresen csatlakoztál!', ephemeral: true });
      });
...
const activities = [
  ...
  { name: 'regedit_404', type: ActivityType.Listening },
  { name: 'ConnectHub Community szerver', type: ActivityType.Watching },
];
